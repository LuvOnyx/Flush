...
private:
    ...
    // Wheel events are NOT routed by processMouseWheel automatically — the
    // Visage window handler explicitly hit-tests the deepest frame and walks UP
    // the parent chain until a frame consumes the wheel (see
    // visage_app/window_event_handler.cpp :: handleMouseWheel). We mirror that
    // here, otherwise the EQ display's wheel-based Q/gain editing never fires.
    void dispatchMouseWheel (const juce::MouseEvent& e, const juce::MouseWheelDetails& wheel) {
        if (!event_root_)
            return;
        ...
        me.position = windowPos;
        me.relative_position = windowPos;
        me.window_position = windowPos;
        ...
        // Normalise to Visage's "positive = scroll up" convention. JUCE's
        // deltaY sign is relative to the user's reversed-wheel preference, so
        // un-reverse it before handing it to the UI.
        me.wheel_delta_y = wheel.isReversed ? -wheel.deltaY : wheel.deltaY;
        me.wheel_delta_x = wheel.isReversed ? -wheel.deltaX : wheel.deltaX;
        me.precise_wheel_delta_y = me.wheel_delta_y;
        me.precise_wheel_delta_x = me.wheel_delta_x;
        me.wheel_momentum = wheel.isInertial;
        me.wheel_reversed = false;

        visage::Frame* frame = event_root_->frameAtPoint (windowPos);
        while (frame) {
            while (frame && frame->ignoresMouseEvents())
                frame = frame->parent();
            if (!frame)
                break;

            me.event_frame = frame;
            me.position = windowPos - frame->positionInWindow();
            if (frame->processMouseWheel (me))
                return;
            frame = frame->parent();
        }
    }
