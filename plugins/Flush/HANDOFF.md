...
3b. **`Brush::vertical` gradients + `Color`.** `VisageControls.h` uses
   `visage::Brush::vertical(visage::Color(top), visage::Color(bottom))` for panel /
   knob / meter / analyzer backgrounds, and `Color::withAlpha()` / `interpolateWith()` /
   `toARGB()` for theming and per-band colours. These are all in the verified
   `visage_graphics` API (`gradient.h` / `color.h`). If a symbol is missing at compile
   time, check the visage submodule is at the pinned revision `968d780` (`.gitmodules`).
