                                        ...
                                        juce::MidiBuffer&)
{
    ...
    // Gain reduction: broadband engine, or spectral dynamics' peak bin GR.
...
void FlushAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    try {
        ...
        // Split off the band tree before replacing parameter state. Both the
        // copy AND ensureBandCount touch bandTree, so do them under one lock.
        {
            std::lock_guard<std::mutex> lock (bandMutex_);
