...
public:
    ...
    juce::ValueTree bandTree { "BANDS" };   // 24 x "BAND" children (source of truth)
    ...
    juce::ValueTree getBand (int index) const;
    ...
    //--- Mark the band graph dirty (UI node edits) — safe from the UI thread -
    void markBandsDirty() { bandsDirty_ = true; linearFirDirty_ = true; }
...
private:
    ...
    std::array<juce::uint32, kMaxBands> bandHash_ {};
    bool bandsDirty_ = true;
    ...
    // Spectrum analyzer (observes the input; display only).
