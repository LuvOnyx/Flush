...
public:
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        double prevDb = 0.0; float prevX = px, prevY = py + ph * 0.5f;
...
// ============================================================ Preset browser ==
class FlushPresetBrowser : public visage::Frame
{
public:
    explicit FlushPresetBrowser (FlushAudioProcessor& p) : processor_ (p) {}

    void init() override
    {
        const float dpi = std::max (1.0f, dpiScale());
        title_font_ = makeFont (10.0f, dpi);
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        canvas.setColor (C::hairline);
        canvas.roundedRectangleBorder (1.0f, 1.0f, width() - 2.0f, height() - 2.0f, 4.0f, 1.0f);

        canvas.setColor (C::label);
        canvas.text ("PRESETS", title_font_, visage::Font::kCenter, 0.0f, 6.0f, width(), 12.0f);

        const auto names = processor_.getFactoryPresetNames();
        const float rowH = 18.0f;
        for (int i = 0; i < names.size(); ++i) {
            const float y = 22.0f + i * rowH;
            canvas.setColor (i == selected_ ? C::surface2 : C::bg0);
            canvas.roundedRectangle (4.0f, y, width() - 8.0f, rowH - 2.0f, 3.0f);
            canvas.setColor (i == selected_ ? C::accent : C::textDim);
            canvas.text (names[i].toRawUTF8(), label_font_, visage::Font::kLeft,
                         10.0f, y + 2.0f, width() - 20.0f, rowH - 4.0f);
        }

        // A/B compare buttons.
        const float ay = height() - 24.0f;
        drawAbButton (canvas, 6.0f, ay, "A", abSlot_ == 0);
        drawAbButton (canvas, width() * 0.5f + 2.0f, ay, "B", abSlot_ == 1);
    }
    ...
    void mouseDown (const visage::MouseEvent& e) override
    {
        ...
        const float ay = height() - 24.0f;
        if (y >= ay && y <= ay + 18.0f) {
            const int slot = (x < width() * 0.5f) ? 0 : 1;
            if (abSlot_ == slot) processor_.recallAbSlot (slot);
            else { processor_.storeAbSlot (slot); abSlot_ = slot; }
            redraw();
            return;
        }
        ...
        // Preset rows.
        const float rowH = 18.0f;
        const int row = (int)((y - 22.0f) / rowH);
        if (row >= 0 && row < names.size()) {
            selected_ = row;
            processor_.loadFactoryPreset (row);
        }
        redraw();
    }

private:
    void drawAbButton (visage::Canvas& canvas, float x, float y, const char* label, bool active)
    {
        canvas.setColor (active ? C::accent : C::surface);
        canvas.roundedRectangle (x, y, width() * 0.5f - 8.0f, 18.0f, 3.0f);
        canvas.setColor (active ? C::bg0 : C::textDim);
        canvas.text (label, label_font_, visage::Font::kCenter, x, y + 2.0f, width() * 0.5f - 8.0f, 14.0f);
    }

    FlushAudioProcessor& processor_;
    visage::Font title_font_, label_font_;
    int selected_ = -1;
    int abSlot_ = -1;
};
...
public:
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        canvas.roundedRectangle (1.0f, 1.0f, width() - 2.0f, height() - 2.0f, 4.0f);
        canvas.setColor (C::text);
        canvas.text ("\u2699", label_font_, visage::Font::kCenter, 0.0f, 2.0f, width(), height() - 4.0f);
    }

    void init() override
    {
        const float dpi = std::max (1.0f, dpiScale());
        label_font_ = makeFont (14.0f, dpi);
    }
...
public:
    ...
    void init() override
    {
        ...
        // EQ display (analyzer + draggable nodes).
        ...
        // Preset browser + settings gear + settings modal.
        addChild (std::make_unique<FlushPresetBrowser> (processor));
    ...
    void resized() override
    {
        ...
        // Region widths (header spans full width; body splits 15/42/18/25).
        ...
        // Input knob + meter.
        child (0)->setBounds (inW * 0.5f - 26.0f, header + 8.0f, 52.0f, 78.0f);
        child (1)->setBounds (inW * 0.5f - 16.0f, header + 96.0f, 32.0f, h - header - 120.0f);

        // EQ display.
        child (2)->setBounds (inW, header, eqW, h - header);

        // Compressor knobs (stacked).
        const float compX = inW + eqW;
        for (int i = 0; i < 4; ++i)
            child (3 + i)->setBounds (compX + compW * 0.5f - 22.0f, header + 12.0f + i * 92.0f, 44.0f, 66.0f);

        // Flush Match + output.
        const float outX = compX + compW;
        child (7)->setBounds (outX + 8.0f, header + 8.0f, outW - 16.0f, 34.0f);   // mode
        child (8)->setBounds (outX + outW * 0.5f - 22.0f, header + 56.0f, 44.0f, 66.0f); // target
        child (9)->setBounds (outX + outW * 0.5f - 26.0f, header + 132.0f, 52.0f, 78.0f); // output trim
        child (10)->setBounds (outX + outW - 40.0f, header + 220.0f, 32.0f, h - header - 244.0f); // out meter

        // Header controls (right side).
        child (11)->setBounds (w - 340.0f, 6.0f, 96.0f, 36.0f);   // phase
        child (12)->setBounds (w - 234.0f, 6.0f, 96.0f, 36.0f);   // meter
        child (14)->setBounds (w - 40.0f, 6.0f, 34.0f, 34.0f);    // settings gear

        // Preset browser: left edge, below the header.
        child (13)->setBounds (4.0f, header + 4.0f, inW - 8.0f, h - header - 8.0f);

        // Settings modal overlay (full window when visible).
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        canvas.text ((juce::String ("LATENCY ") + juce::String (latMs, 1) + " MS").toRawUTF8(),
                     label_font_, visage::Font::kTopRight, width() - 380.0f, 18.0f, 150.0f, 12.0f);

        // Live Flush delta (the hero readout).
        ...
        canvas.text ((juce::String ("FLUSH ") + juce::String (delta, 1) + " dB").toRawUTF8(),
                     value_font_, visage::Font::kTopRight, width() - 660.0f, 14.0f, 240.0f, 20.0f);
    }
