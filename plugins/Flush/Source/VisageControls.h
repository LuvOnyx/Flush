...
//   * left-drag a node          -> move frequency (horizontal) + gain (vertical)
//   * left-click empty space    -> select nothing (clears the active node)
//   * DOUBLE-click empty space  -> ADD a bell band at that frequency
//   * DOUBLE-click a node       -> reset that band's gain to 0 dB
//   * RIGHT-click a node        -> context menu (type / dynamic / solo / remove)
//   * RIGHT-click empty space   -> context menu (add band)
...
public:
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        // Refresh the (locked) band snapshot for drawing + hit-testing.
        cachedBands_ = processor_.getBandTreeCopy();
    ...
    void mouseDown (const visage::MouseEvent& e) override
    {
        if (e.isRightButtonCurrentlyDown()) {
            showContextMenu (e);
            return;
        }
        ...
        if (doubleClick) {
            if (hit >= 0) {
                // Double-click a node -> reset its gain.
                auto b = processor_.getBand (hit);
                processor_.moveBand (hit, b.getProperty ("freq", 1000.0), 0.0);
            } else {
                // Double-click empty space -> add a bell band at this frequency.
    ...
    void mouseDrag (const visage::MouseEvent& e) override
    {
        ...
        // Read the band's CURRENT freq/gain from the (locked) source, apply the
        // delta, and write back atomically through the processor.
    ...
    // Keyboard: Delete/Backspace remove the selected band; arrows nudge its
    // frequency/gain (Shift = fine). Returns false for unhandled keys so the
    // host keeps them.
...
private:
    ...
    {
        const float px = 34.0f, pw = width() - px - 10.0f;
        const float t = clamp ((x - px) / std::max (1.0f, pw), 0.0f, 1.0f);
    ...
    void showContextMenu (const visage::MouseEvent& e)
    {
        const int hit = hitTest (e.position.x, e.position.y);
        selected_ = hit;
        // Capture these BY VALUE — the lambda outlives this function (and `e`).
        const float addFreq = freqAtX (e.position.x);
        const visage::Point menuPos = e.position;

        // Band type names, indexed by flush::Shape enum order.
...
public:
    ...
    void init() override
    {
        ...
        // 3: EQ display (analyzer + draggable nodes). Kept as a raw pointer so
        // the root can forward keyboard events to it (the host sends keys to the
        // root frame only).
    ...
    // Keyboard events arrive on the ROOT frame only (the host forwards JUCE key
    // presses to the event root); route them to the EQ display.
    bool keyPress (const visage::KeyEvent& e) override
    {
