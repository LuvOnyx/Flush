...
public:
    ...
    void draw (visage::Canvas& canvas) override
    {
        ...
        const auto names = processor_.getAllPresetNames();
        ...
        canvas.text (name.toRawUTF8(), value_font_, visage::Font::kTopLeft,
                     66.0f, 15.0f, std::max (40.0f, std::min (200.0f, width() - 360.0f)), 15.0f);

        // SAVE (user preset), A, B on the right.
        const float abX = width() - 110.0f;
        drawSave (canvas, abX - 52.0f);
    ...
    void mouseDown (const visage::MouseEvent& e) override
    {
        ...
        else if (x >= abX - 52.0f && x < abX - 2.0f) { save(); }
        else if (x >= abX && x < abX + 48.0f) {
...
private:
    void prev()
    {
        ...
        processor_.loadPresetByName (names[current_]);
    }
    ...
    void save()
    {
        // Snapshot as "User N" (no text entry widget in the Visage frame tree).
        const auto user = processor_.getUserPresetNames();
        juce::String name;
        int n = 1;
        do { name = "User " + juce::String (n++); } while (user.contains (name));
        processor_.saveUserPreset (name);
    }

    void drawSave (visage::Canvas& canvas, float x)
    {
        canvas.setColor (C::surface2);
        canvas.roundedRectangle (x, 5.0f, 48.0f, 16.0f, 3.0f);
        canvas.setColor (C::textDim);
        canvas.text ("SAVE", label_font_, visage::Font::kCenter, x, 6.0f, 48.0f, 14.0f);
    }
