...
public:
    ...
    {
        baseFs_ = baseFs;
        const auto lp = windowedSincLowpass (baseFs * 2.0, baseFs * 0.45, 95, 6.0);
        up_.set (lp);
        down_.set (lp);
    }
    ...
    {
        down_.process (in0);
        return down_.process (in1);
    }

    int latencySamples() const { return up_.latency(); }   // 2x-rate samples
...
public:
    ...
    // (s1 + s2) up + (d2 + d1) down, each stage's latency in its own rate.
    int latencySamples() const { return s1_.latencySamples() + s2_.latencySamples(); }
